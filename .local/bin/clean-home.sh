...
if [[ "$CLEAN_HOME_OPTIONAL" =~ [yY] ]]; then
  rm -rf $HOME/.angular-config.json $HOME/.cache/commitizen $HOME/.cache/github-copilot $HOME/.cache/gh $HOME/.cache/git $HOME/.cache/google-chrome $HOME/.cache/JetBrains $HOME/.cache/JNA $HOME/.cache/Microsoft $HOME/.cache/starship $HOME/.codex $HOME/.config/Code $HOME/.config/gh $HOME/.config/git $HOME/.config/google-chrome $HOME/.config/JetBrains $HOME/.gradle $HOME/.java $HOME/.lemminx $HOME/.local/share/JetBrains $HOME/.m2 $HOME/.npm $HOME/.nvm $HOME/.sdkman $HOME/.sts4 $HOME/.testcontainers.properties $HOME/.vscode $HOME/workspace
else
